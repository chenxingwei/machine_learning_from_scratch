...
![equation](http://latex.codecogs.com/gif.latex?\hat{y_i}=\sum_{j=1}^{m}w_jx_{ij}+b=\theta^{T}x_i)

Then we re-formula the loss function as

![equation](http://latex.codecogs.com/gif.latex?L=\frac{1}{n}\sum_{i=1}^{n}(y_i-Wx_i^T-b)^2=\frac{1}{n}(\theta^{T}x_i-y_i)^2)
