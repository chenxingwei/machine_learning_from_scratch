...
Then for each dependent variable, we could model is with the independent variables as ![equation](http://latex.codecogs.com/gif.latex?\hat{y_i}=\sum_{j=1}^{m}w_jx_{ij}+b=Wx_i^T+b), where ![equation](http://latex.codecogs.com/gif.latex?W=[w_1,w_2,...,w_m])
